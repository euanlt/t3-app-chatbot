...
export const mcpRouter = createTRPCRouter({
  ...
  getActiveTools: publicProcedure
    .query(async () => {
      ...
      const activeServers = Array.from(userServers.values()).filter(s => s.status === "active");
  ...
  getResource: publicProcedure
    ...
    .query(async ({ input }) => {
      logger.info("Getting resource", { serverId: input.serverId, uri: input.uri });
      
      try {
        const result = await mcpClient.getResource(input.serverId, input.uri);
        logger.info("Fetched resource", { serverId: input.serverId, uri: input.uri });
        return result;
      } catch (error) {
  ...
  searchTools: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        logger.info("Searched tools", { query: input.query, count: tools.length });
        return tools;
      } catch (error) {
