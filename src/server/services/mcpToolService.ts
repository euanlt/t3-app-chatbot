...
export class MCPToolService {
  ...
  async analyzeMessageForTools(message: string): Promise<ToolCall[]> {
    ...
    for (const { serverId, serverName, tool } of availableTools) {
      ...
      if (tool.name === "search" || tool.name === "tavily_search" || tool.name === "tavily-search") {
        if (lowerMessage.includes("search") || 
          ...
          args.query = message.replace(/^(search|find|look up|what is|who is|tell me about)/i, "").trim();
        }
      }
   ...
   */
  async getToolRecommendations(message: string): Promise<Array<{ tool: string; description: string; server: string }>> {
    const recommendations: Array<{ tool: string; description: string; server: string }> = [];
    const lowerMessage = message.toLowerCase();
