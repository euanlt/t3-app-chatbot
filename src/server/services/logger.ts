...
const currentLevel = process.env.LOG_LEVEL
  ? (LogLevel[process.env.LOG_LEVEL.toUpperCase() as LogLevelString] ??
    LogLevel.WARN)
  : LogLevel.WARN;
  ...
  if (typeof obj === "string") {
    ...
    for (const key in obj) {
      ...
      if (isSensitiveKey) {
        ...
        result[key] = deepRedactSensitiveData(
    ...
    if (currentLevel >= LogLevel.ERROR) {
      ...
      if (error) {
        if (error instanceof Error && error.stack) {
          ...
          console.error(
            `${COLORS.RED}${String(redactSensitiveData(error.stack))}${COLORS.RESET}`,
          );
...
export { redactSensitiveData, deepRedactSensitiveData };
